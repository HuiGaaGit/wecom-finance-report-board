...
import { bindCashFlowAnalysis, cashFlowAnalysisButtonHtml } from './cash-flow-analysis.js';
...
const consultantSpendRevenueReportType = 'consultant_spend_revenue';
...
const consultantRoiView = {
  ...
  reimbursementAccounts: new Set(),
  savedReimbursementAccounts: new Set(),
  reimbursementPeriod: '',
  settingsDirty: false,
  filters: {},
  sortKey: 'output',
...
const consultantDirectoryPollingMs = 3_000;
...
let consultantDirectoryPollingUntil = 0;
let consultantDirectoryGuideSeenKey = '';
...
function renderNav() {
  ...
  nav.innerHTML = modules.map(module => {
    ...
    const handle = canReorder ? `<span class="nav-drag-handle" data-nav-drag-handle title="拖动调整全局模块顺序（应用于所有公司）" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i></span>` : '';
...
function bindSidebarModuleOrder(nav) {
  ...
  const finish = event => {
    ...
    api('/api/admin/module-order', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ order }) })
      .then(result => { state.bootstrap.moduleOrder = result.moduleOrder; state.bootstrap.moduleOrderScope = result.scope || 'all_companies'; showNotice('左侧模块顺序已保存，已应用于所有公司和员工'); refresh(); })
...
const journalDisplayAliases = { date: ['日期'], voucher: ['凭证号'], summary: ['摘要'], accountCode: ['科目编码'], account: ['科目名称'], debit: ['借方金额', '借方本币'], credit: ['贷方金额', '贷方本币'] };
const journalDisplayColumns = raw => {
  const normalized = value => String(value ?? '').normalize('NFKC').replace(/\s+/g, '').trim();
  const header = (raw?.rows || []).find(row => { const values = (row.cells || []).map(normalized); return values.includes('科目名称') && (values.includes('摘要') || values.includes('凭证号')); })?.cells || [];
  const index = (names, fallback) => { const wanted = new Set(names.map(normalized)); const found = header.findIndex(value => wanted.has(normalized(value))); return found >= 0 ? found : fallback; };
  return Object.fromEntries(Object.entries(journalDisplayAliases).map(([key, names]) => [key, index(names, { date: 0, voucher: 1, summary: 2, accountCode: 3, account: 4, debit: 5, credit: 6 }[key])]));
};
const journalDisplayRow = (row, columns = null) => {
  if (row?.journal) return row.journal;
  const c = row?.cells || []; const map = columns || { date: 0, voucher: 1, summary: 2, accountCode: 3, account: 4, debit: 5, credit: 6 };
  return { date: c[map.date], voucher: c[map.voucher], summary: c[map.summary], accountCode: c[map.accountCode], account: c[map.account], debit: Number(c[map.debit] || 0), credit: Number(c[map.credit] || 0) };
};
...
function journalRowsTableHtml(rows, showDirection = true) { return rows?.length ? `<div class="table-wrap"><table class="data-table journal-detail-table${showDirection ? '' : ' no-direction'}"><colgroup><col class="journal-voucher"><col class="journal-account"><col class="journal-summary">${showDirection ? '<col class="journal-direction">' : ''}<col class="journal-amount"></colgroup><thead><tr><th>凭证号</th><th>科目名称</th><th>摘要</th>${showDirection ? '<th>借贷方向</th>' : ''}<th>金额</th></tr></thead><tbody>${rows.map(row => { const item = journalDisplayRow(row); const debit = Number(item.debit || 0); const credit = Number(item.credit || 0); const direction = debit && credit ? '借/贷' : debit ? '借' : credit ? '贷' : ''; const amount = debit && credit ? `${statementAmount(debit)} / ${statementAmount(credit)}` : debit ? statementAmount(debit) : credit ? statementAmount(credit) : '-'; return `<tr><td>${escapeHtml(item.voucher)}</td><td>${escapeHtml(item.account)}</td><td class="summary-cell">${escapeHtml(item.summary)}</td>${showDirection ? `<td class="journal-direction direction-cell">${escapeHtml(direction)}</td>` : ''}<td class="num amount-cell">${escapeHtml(amount)}</td></tr>`; }).join('')}</tbody></table></div>` : '<div class="empty">暂无匹配的序时账明细。</div>'; }
...
const financialBriefPlainText = brief => {
  const [year, month] = String(brief.period || '').split('-'); const secondaryItems = brief.secondaryItems || [];
  return [
...
const scheduleConsultantRoiAutoRefresh = () => {
  ...
  }, consultantDirectoryPollingUntil > Date.now() ? consultantDirectoryPollingMs : consultantRoiAutoRefreshMs);
};
...
let businessJournalDetailStore = new Map();
const businessJournalDetailCell = (value, details, title, canView, showDirection) => {
  const formatted = businessCurrency(value);
  if (!canView || !details?.length || Math.abs(Number(value || 0)) < .000001) return formatted;
  const key = `business-journal-${businessJournalDetailStore.size + 1}`;
  businessJournalDetailStore.set(key, { title, details, showDirection });
  return `<button type="button" class="expense-drill business-journal-drill" data-business-journal-detail="${key}" title="点击查看具体序时账分录">${formatted}</button>`;
};
const openBusinessJournalDetail = key => {
  const item = businessJournalDetailStore.get(key); const modal = $('#business-journal-detail-modal');
  if (!item || !modal) return;
  const rows = item.details || []; const directionHeader = item.showDirection ? '<th>借方</th><th>贷方</th>' : '<th>金额</th>';
  const body = rows.map(row => {
    const directionCells = item.showDirection ? `<td class="num">${statementAmount(row.debit)}</td><td class="num">${statementAmount(row.credit)}</td>` : `<td class="num">${statementAmount(row.amount)}</td>`;
    return `<tr><td>${escapeHtml(row.date)}</td><td>${escapeHtml(row.voucher)}</td><td class="expense-detail-summary">${escapeHtml(row.summary)}</td><td>${escapeHtml(row.accountCode)}</td><td>${escapeHtml(row.account)}</td>${directionCells}<td class="num">${Number(row.sourceRow || 0) || '—'}</td></tr>`;
  }).join('');
  modal.querySelector('.expense-detail-modal-title').textContent = item.title;
  modal.querySelector('.expense-detail-modal-sub').textContent = `共 ${rows.length} 条构成分录；仅展示参与该金额计算的序时账行，不含结转分录`;
  modal.querySelector('.expense-detail-modal-body').innerHTML = `<div class="table-wrap"><table class="data-table expense-detail-table business-journal-detail-table"><thead><tr><th>日期</th><th>凭证号</th><th>摘要</th><th>科目编码</th><th>科目名称</th>${directionHeader}<th>源表行</th></tr></thead><tbody>${body}</tbody></table></div>`;
  modal.classList.remove('hidden');
};
const bindBusinessJournalDetail = () => {
  document.querySelectorAll('[data-business-journal-detail]').forEach(button => button.onclick = () => openBusinessJournalDetail(button.dataset.businessJournalDetail));
  $('#business-journal-detail-close')?.addEventListener('click', () => $('#business-journal-detail-modal').classList.add('hidden'));
  $('#business-journal-detail-modal')?.addEventListener('click', event => { if (event.target.id === 'business-journal-detail-modal') event.currentTarget.classList.add('hidden'); });
};
...
async function renderMainBusinessAnalysis() {
  ...
  try {
    ...
    const currentMonthText = periodMonthText(analysis.period || scope.period); const previousMonthText = periodMonthText(analysis.previousPeriod);
    const journalSourceText = `序时账：${analysis.source?.fileName || '—'} · ${analysis.source?.sourceSheet || '—'}${analysis.source?.demo ? ' · 演示原始资料' : ` · 上传批次 ${analysis.source?.uploadKey || '—'}`}`;
    const quotationSourceText = analysis.quotationSource?.noData
      ? '集团报单表：尚未发布'
      : `集团报单表：${analysis.quotationSource?.fileName || '—'} · 版本 ${analysis.quotationSource?.sourceVersion || '未标注'} · ${analysis.quotationSource?.recordCount || 0} 条合同 · 本期命中 ${analysis.quotationMatch?.matchedContracts || 0}/${analysis.quotationMatch?.currentContracts || 0}`;
    const sourceText = `${journalSourceText}；${quotationSourceText}`;
    businessJournalDetailStore = new Map();
    const detailHtml = detailRows.map(row => { const subject = `${row.contractNo} · ${row.customerName || '未识别客户'} · ${row.projectName}`; return `<tr><td class="num">${row.index}</td><td>${escapeHtml(row.contractNo)}</td><td>${escapeHtml(row.customerName || '未识别客户')}</td><td>${escapeHtml(row.projectName)}</td><td class="num">${businessJournalDetailCell(row.revenue, row.revenueDetails, `${subject} · 确认收入`, analysis.canViewJournalDetails, analysis.showJournalDirection)}</td><td class="num">${businessJournalDetailCell(row.cost, row.costDetails, `${subject} · 确认成本`, analysis.canViewJournalDetails, analysis.showJournalDirection)}</td><td class="num ${row.grossMargin !== null && row.grossMargin < 0 ? 'negative' : 'positive'}">${businessRate(row.grossMargin)}</td></tr>`; }).join('');
    ...
    if (analysis.canViewJournalDetails) page.querySelector('.business-detail-table')?.closest('.table-wrap')?.insertAdjacentHTML('afterend', '<div class="standard-hint">点击确认收入或确认成本金额，可查看构成该数字的具体序时账分录。</div>');
    ...
    const projectHeaders = projectGrid?.querySelectorAll('.business-project-table thead th') || [];
    if (projectHeaders[1]) { projectHeaders[1].textContent = `${currentMonthText}项目数量`; projectHeaders[1].title = analysis.period || scope.period; }
    if (projectHeaders[2]) { projectHeaders[2].textContent = `${previousMonthText}项目数量`; projectHeaders[2].title = analysis.previousPeriod || ''; }
    const projectSubtitle = projectGrid?.querySelector('.panel-sub'); if (projectSubtitle) projectSubtitle.textContent = `按项目名称归类；条形长度按${currentMonthText}与${previousMonthText}最大项目数统一缩放，数字为精确项目数`;
    projectGrid?.querySelector('.toolbar')?.insertAdjacentHTML('beforeend', `<div class="project-table-legend"><span><i class="legend-dot current"></i>${escapeHtml(currentMonthText)}</span><span><i class="legend-dot previous"></i>${escapeHtml(previousMonthText)}</span></div>`);
    ...
    if (analysis.canViewJournalDetails) page.insertAdjacentHTML('beforeend', '<div id="business-journal-detail-modal" class="expense-detail-modal hidden" role="dialog" aria-modal="true" aria-labelledby="business-journal-detail-title"><section class="expense-detail-dialog"><button id="business-journal-detail-close" class="expense-detail-close" type="button" aria-label="关闭序时账明细">×</button><h3 id="business-journal-detail-title" class="expense-detail-modal-title">序时账明细</h3><div class="expense-detail-modal-sub"></div><div class="expense-detail-modal-body"></div></section></div>');
    bindCommonFilters(); bindBusinessJournalDetail();
...
const consultantRoiColumns = [
  ...
  { key: 'matchLabel', label: '匹配状态', type: 'status', adminOnly: true }
];
const consultantRoiMatchLabel = status => status === 'matched' ? '已匹配' : status === 'no_revenue' ? '无营收' : status === 'missing_payroll' ? '缺工资' : status === 'missing_revenue' ? '缺营收' : '';
const consultantRoiSourceText = row => [
  ...
  ...(row.spendDetails || []).map(item => `${item.sourceSheet}第${item.row}行${item.englishName}`)
].join('；');
...
const consultantRoiSelectedReimbursement = row => {
  const details = row.expenseDetails || [];
  if (!details.length) return Number(row.journalExpense || 0);
  const selected = consultantRoiView.reimbursementAccounts;
  return details.reduce((sum, detail) => sum + (selected.has(detail.secondaryAccount || '未分类') ? Number(detail.amount || 0) : 0), 0);
};
const consultantRoiSettingsAreDirty = () => {
  if (consultantRoiInputDefinitions.some(item => consultantRoiView.inputs[item.key] !== consultantRoiView.savedInputs[item.key])) return true;
  if (consultantRoiView.reimbursementAccounts.size !== consultantRoiView.savedReimbursementAccounts.size) return true;
  return [...consultantRoiView.reimbursementAccounts].some(name => !consultantRoiView.savedReimbursementAccounts.has(name));
};
const consultantRoiRowsWithSelectedInputs = rawRows => (rawRows || []).map(row => {
    const adjusted = { ...row, journalExpense: consultantRoiSelectedReimbursement(row) };
    const input = consultantRoiInputDefinitions.reduce((sum, item) => sum + (consultantRoiView.inputs[item.key] ? Number(adjusted[item.key] || 0) : 0), 0);
    const output = Number(row.output || 0); const matchLabel = consultantRoiMatchLabel(row.matchStatus);
    return { ...adjusted, input, output, roi: input ? output / input : null, matchLabel, sourceText: consultantRoiSourceText(row) };
  });
const consultantRoiRowsForView = rawRows => {
  ...
  const column = consultantRoiColumns.find(item => item.key === consultantRoiView.sortKey) || consultantRoiColumns.find(item => item.key === 'output'); const direction = consultantRoiView.sortDirection === 'asc' ? 1 : -1;
...
const consultantRoiAverageSummary = rawRows => {
  ...
  const averageRoi = valid.length ? valid.reduce((sum, row) => sum + row.roi, 0) / valid.length : null; const companies = new Map();
  const ensureCompany = name => { const key = String(name || '待补充').trim() || '待补充'; if (!companies.has(key)) companies.set(key, { companyName: key, input: 0, output: 0, consultants: new Set() }); return companies.get(key); };
  for (const row of rows) {
    const item = ensureCompany(row.companyName); item.input += row.input; item.output += row.output; item.consultants.add(row.canonicalName || row.name);
  }
  return { averageRoi, consultantCount: valid.length, companies: [...companies.values()].map(item => ({ companyName: item.companyName, input: item.input, output: item.output, roi: item.input ? item.output / item.input : null, consultantCount: item.consultants.size })).sort((a, b) => b.output - a.output || b.input - a.input) };
};
const consultantRoiTrendChartHtml = points => {
  if (!points?.length) return '<div class="empty">本年度当前月份及以前暂无已发布营收数据</div>';
  const width = Math.max(620, points.length * 92); const height = 310; const plot = { left: 66, right: 66, top: 30, bottom: 48 };
  const plotWidth = width - plot.left - plot.right; const plotHeight = height - plot.top - plot.bottom;
  const maxRevenue = Math.max(...points.map(item => Number(item.expectedRevenue || 0)), 1); const validRois = points.map(item => item.roi).filter(value => Number.isFinite(value)); const maxRoi = Math.max(...validRois, 1);
  const xFor = index => plot.left + (points.length === 1 ? plotWidth / 2 : index * plotWidth / (points.length - 1));
  const revenueY = value => plot.top + plotHeight - Number(value || 0) / maxRevenue * plotHeight;
  const roiY = value => plot.top + plotHeight - Number(value || 0) / maxRoi * plotHeight;
  const revenuePath = points.map((item, index) => `${index ? 'L' : 'M'}${xFor(index).toFixed(1)},${revenueY(item.expectedRevenue).toFixed(1)}`).join(' ');
  let roiPath = ''; let active = false;
  points.forEach((item, index) => { if (!Number.isFinite(item.roi)) { active = false; return; } roiPath += `${active ? ' L' : ' M'}${xFor(index).toFixed(1)},${roiY(item.roi).toFixed(1)}`; active = true; });
  const grid = [0, .25, .5, .75, 1].map(rate => { const y = plot.top + plotHeight * (1 - rate); return `<line x1="${plot.left}" y1="${y}" x2="${width - plot.right}" y2="${y}" class="consultant-trend-grid"/><text x="${plot.left - 10}" y="${y + 4}" text-anchor="end" class="consultant-trend-axis">${escapeHtml(money(maxRevenue * rate))}</text><text x="${width - plot.right + 10}" y="${y + 4}" class="consultant-trend-axis">${(maxRoi * rate).toFixed(1)}倍</text>`; }).join('');
  const labels = points.map((item, index) => `<text x="${xFor(index)}" y="${height - 16}" text-anchor="middle" class="consultant-trend-month">${escapeHtml(String(item.period || '').slice(5))}月</text>`).join('');
  const revenuePoints = points.map((item, index) => `<circle cx="${xFor(index)}" cy="${revenueY(item.expectedRevenue)}" r="4.5" class="consultant-trend-point revenue"><title>${escapeHtml(item.period)} 预计营收 ${escapeHtml(money(item.expectedRevenue))}</title></circle>`).join('');
  const roiPoints = points.map((item, index) => Number.isFinite(item.roi) ? `<circle cx="${xFor(index)}" cy="${roiY(item.roi)}" r="4.5" class="consultant-trend-point roi"><title>${escapeHtml(item.period)} 投入产出比 ${item.roi.toFixed(2)} 倍</title></circle>` : '').join('');
  return `<div class="consultant-trend-legend"><span class="revenue">预计营收</span><span class="roi">投入产出比</span></div><div class="consultant-trend-chart-scroll" role="img" aria-label="顾问预计营收和投入产出比月度趋势图"><svg class="consultant-trend-chart" viewBox="0 0 ${width} ${height}" style="min-width:${width}px">${grid}<path d="${revenuePath}" class="consultant-trend-line revenue"/>${roiPath ? `<path d="${roiPath.trim()}" class="consultant-trend-line roi"/>` : ''}${revenuePoints}${roiPoints}${labels}</svg></div><p class="consultant-trend-note">左轴为预计营收，右轴为投入产出比；仅展示本年度已发布营收数据的月份。</p>`;
};
const consultantRevenueDetailsHtml = data => `<div class="consultant-revenue-summary"><span>${escapeHtml(data.period)} · 共 ${Number(data.count || 0)} 条营收明细</span><strong>预计营收合计 ${money(data.total)}</strong></div><div class="table-wrap consultant-revenue-detail-wrap"><table class="data-table consultant-revenue-detail-table"><thead><tr><th>合同编号</th><th>项目名称</th><th>客户名字</th><th>预计营收额</th></tr></thead><tbody>${(data.details || []).map(item => `<tr><td>${escapeHtml(item.contractNo || '—')}</td><td>${escapeHtml(item.projectName || '—')}</td><td>${escapeHtml(item.customerName || '—')}</td><td class="num">${money(item.expectedRevenue)}</td></tr>`).join('') || '<tr><td colspan="4" class="empty">该顾问本月暂无可展示的营收明细</td></tr>'}</tbody></table></div>`;
const consultantRoiFilterControlHtml = column => column.type === 'status'
  ? `<select data-roi-filter-draft="${column.key}" aria-label="筛选${column.label}"><option value="">全部状态</option>${['已匹配', '无营收', '缺营收', '缺工资'].map(value => `<option value="${value}" ${consultantRoiView.filters[column.key] === value ? 'selected' : ''}>${value}</option>`).join('')}</select>`
  : `<input data-roi-filter-draft="${column.key}" value="${escapeHtml(consultantRoiView.filters[column.key] || '')}" aria-label="筛选${column.label}" placeholder="${column.type === 'number' ? '例如 ≥10000 或 5000-20000' : `输入${column.label}关键词`}">`;
const consultantRoiFilterIcon = '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M2.2 3.1h11.6L9.4 8v4.1l-2.8 1.4V8L2.2 3.1Z"/></svg>';
const consultantDirectoryGuideFor = sync => {
  const guides = {
    not_configured: { tone: 'warning', title: '企业微信自动匹配尚未配置', steps: ['请管理员在财务服务器安装顾问人事同步 service、path 和 timer', '以服务运行账号完成 wecom-cli auth init', '手工运行一次同步并确认状态成功', '启用 path 后，后续报表发布将自动刷新'] },
    pending: { tone: 'working', title: '正在等待自动匹配', steps: ['服务器已收到刷新请求', '系统将读取企微花名册和通讯录', '页面会自动更新，无需重复点击'] },
    running: { tone: 'working', title: '正在读取企业微信', steps: ['正在匹配顾问姓名、英文名、所属公司和离职状态', '读取完成后页面会自动刷新'] },
    auth_required: { tone: 'warning', title: '企业微信授权需要更新', steps: ['财务系统已检测到独立授权到期', '财务管理员点击下方临时链接并在企业微信中确认', '授权完成后页面会自动复检并刷新顾问匹配', '链接失效时点击“重新生成授权链接”'] },
    source_scope_required: { tone: 'warning', title: '花名册读取范围需要调整', steps: ['系统只允许读取在职/离职表 D:F 的所属公司、姓名、英文名，以及离职表 B 列的离职日期', '当前响应的起始列或有效宽度超出白名单，系统已停止且不会覆盖旧快照', '请管理员核对列位置和表头后返回本页点击“重新匹配”'] },
    source_permission_required: { tone: 'warning', title: '企微文档或通讯录暂时无法读取', steps: ['确认在线表格名称为“花名册”，并包含“在职”“离职”工作表', '确认 D:F 表头为所属公司、姓名、英文名，离职表 B 列为离职日期', '向财务专用企微同步账号授予花名册和通讯录查看权限；如有多个同名花名册，请配置唯一文档链接', '完成后返回本页点击“重新匹配”'] },
    source_data_required: { tone: 'warning', title: '缺少顾问匹配名单', steps: ['先上传并发布本月工资表', '确认工资表包含部门和姓名，且顾问部门可被识别', '发布成功后系统会自动重新匹配'] },
    error: { tone: 'error', title: '自动匹配服务需要检查', steps: ['请管理员检查 wecom-finance-consultant-directory.service 状态和最近日志', '确认服务器网络及企微连接正常', '处理后返回本页点击“重新匹配”'] }
  };
  return guides[sync?.state] || null;
};
const consultantDirectoryGuideHtml = (sync, guide) => !guide ? '' : `<div class="consultant-directory-guide ${guide.tone}"><div><strong>${escapeHtml(guide.title)}</strong><span>${escapeHtml(sync?.message || '')}</span></div><button type="button" class="button" id="consultant-directory-guide-open">查看处理指引</button></div>`;
const requestConsultantDirectoryRefresh = async () => {
  const button = $('#consultant-roi-refresh'); if (button) { button.disabled = true; button.classList.add('refreshing'); button.innerHTML = '<span aria-hidden="true">↻</span>提交中…'; }
  try {
    await api('/api/analysis/consultant-directory/refresh', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ companyKey: state.company, period: state.period }) });
    consultantDirectoryPollingUntil = Date.now() + 45_000; showNotice('已提交企微花名册与通讯录匹配，完成后将自动更新');
    await renderConsultantRoiInteractive({ trigger: 'manual' });
  } catch (error) { showNotice(error.message, true); if (button?.isConnected) { button.disabled = false; button.classList.remove('refreshing'); button.innerHTML = '<span aria-hidden="true">↻</span>重新匹配'; } }
};
const requestConsultantDirectoryAuthorization = async () => {
  const button = $('#consultant-directory-auth-request'); if (button) { button.disabled = true; button.textContent = '正在生成…'; }
  try {
    await api('/api/analysis/consultant-directory/authorize', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' });
    consultantDirectoryPollingUntil = Date.now() + 16 * 60_000; showNotice('已申请新的企业微信授权链接，生成后将自动显示');
    await renderConsultantRoiInteractive({ trigger: 'authorization' });
  } catch (error) { showNotice(error.message, true); if (button?.isConnected) { button.disabled = false; button.textContent = '重新生成授权链接'; } }
};
...
async function renderConsultantRoiInteractive({ trigger = 'initial' } = {}) {
  ...
  try {
    ...
    if (!data.canViewMatchDiagnostics) { delete consultantRoiView.filters.matchLabel; if (consultantRoiView.sortKey === 'matchLabel') { consultantRoiView.sortKey = 'output'; consultantRoiView.sortDirection = 'desc'; } }
    const reimbursementOptions = data.reimbursementAccounts || []; const reimbursementNames = reimbursementOptions.map(item => item.name);
    const savedInputs = { ...consultantRoiView.inputs, ...(data.roiSettings?.inputs || {}) };
    const savedAccounts = new Set(data.canManageRoiSettings ? (data.roiSettings?.selectedAccounts || []) : []);
    consultantRoiView.reimbursementPeriod = data.period; consultantRoiView.savedInputs = { ...savedInputs }; consultantRoiView.inputs = { ...savedInputs };
    consultantRoiView.savedReimbursementAccounts = new Set(savedAccounts); consultantRoiView.reimbursementAccounts = new Set(savedAccounts); consultantRoiView.settingsDirty = false;
    const staleReimbursementText = data.canManageRoiSettings && data.roiSettings?.staleAccounts?.length ? `已保存的报销科目“${data.roiSettings.staleAccounts.join('、')}”在当前数据源中不存在，已忽略；请管理员核对后重新保存。` : '';
    const sourceState = data.missing?.length ? `<span class="consultant-roi-missing"><strong>来源待补齐</strong><span>${data.missing.map(escapeHtml).join('、')}</span></span>` : '<span class="consultant-roi-complete">四类数据来源已匹配</span>';
    ...
    const revenueFields = data.sources.revenueFields || {}; const revenueFieldText = [revenueFields.period ? `期间→${revenueFields.period}` : '', revenueFields.consultant ? `顾问→${revenueFields.consultant}` : '', revenueFields.expectedRevenue ? `预计营收→${revenueFields.expectedRevenue}` : ''].filter(Boolean).join('；');
    const spendFields = data.sources.spendFields || {}; const spendFieldText = [spendFields.englishName ? `英文名→${spendFields.englishName}` : '', spendFields.trafficSpend ? `投流消耗→${spendFields.trafficSpend}` : ''].filter(Boolean).join('；');
    const spendIssueText = data.canViewMatchDiagnostics && Number(data.sources.unmatchedSpendRows || 0) ? `投流表另有 ${Number(data.sources.unmatchedSpendRows)} 行、${money(data.sources.unmatchedSpendAmount)} 元未能按英文名唯一匹配${Number(data.sources.ambiguousSpendRows || 0) ? `（其中 ${Number(data.sources.ambiguousSpendRows)} 行英文名有歧义）` : ''}，未计入任何顾问。` : '';
    ...
    const directorySource = data.sources.directory || {}; const directorySync = directorySource.sync || {}; const directoryGuide = consultantDirectoryGuideFor(directorySync);
    const directoryText = directorySource.available ? `人事：企业微信花名册与通讯录，匹配姓名、英文名、所属公司及离职状态共 ${Number(directorySource.matchedRows || 0)} 人${directorySource.generatedAt ? `，同步于 ${directorySource.generatedAt}` : ''}` : `人事：${directorySource.reason || '尚未同步企业微信花名册与通讯录'}`;
    if (directorySync.state === 'auth_required' && directorySync.authUrl) consultantDirectoryPollingUntil = Math.max(consultantDirectoryPollingUntil, Date.now() + 16 * 60_000);
    else if (!['pending', 'running'].includes(directorySync.state) && consultantDirectoryPollingUntil <= Date.now()) consultantDirectoryPollingUntil = 0;
    const authExpiresText = directorySync.authUrlExpiresAt ? new Date(directorySync.authUrlExpiresAt).toLocaleTimeString('zh-CN', { hour12: false, hour: '2-digit', minute: '2-digit' }) : '';
    const authorizationActions = directorySync.state === 'auth_required'
      ? data.canManageAuthorization
        ? `${directorySync.authUrl ? `<a class="button primary consultant-directory-auth-link" id="consultant-directory-auth-link" href="${escapeHtml(directorySync.authUrl)}" target="_blank" rel="noopener noreferrer">立即重新授权</a>` : ''}<button type="button" class="button" id="consultant-directory-auth-request">${directorySync.authUrl ? '重新生成授权链接' : '生成授权链接'}</button>`
        : '<span class="consultant-directory-admin-only">请联系财务管理员更新授权</span>'
      : '<button type="button" class="button primary" id="consultant-directory-guide-retry">重新匹配</button>';
    const refreshedAt = new Date().toLocaleTimeString('zh-CN', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });
    const visibilityButton = data.canManageVisibility ? `<button type="button" class="button consultant-roi-visibility-open" id="consultant-roi-visibility-open">名单显示设置${Number(data.visibility?.hiddenCount || 0) ? `<span>${Number(data.visibility.hiddenCount)}</span>` : ''}</button>` : '';
    const inputTags = consultantRoiInputDefinitions.map(item => {
      if (!data.canManageRoiSettings) return `<span class="consultant-roi-tag readonly ${savedInputs[item.key] ? 'selected' : ''}"><span>${savedInputs[item.key] ? '✓' : '＋'}</span>${escapeHtml(item.label)}</span>`;
      if (item.key === 'journalExpense') return `<span class="consultant-roi-reimbursement-picker"><button type="button" data-roi-input="${item.key}"></button><button type="button" class="consultant-roi-reimbursement-open" id="consultant-roi-reimbursement-open" aria-label="配置报销费用二级科目" aria-expanded="false" aria-controls="consultant-roi-reimbursement-menu">${consultantRoiFilterIcon}</button></span>`;
      return `<button type="button" data-roi-input="${item.key}"></button>`;
    }).join('');
    const settingsApplyButton = data.canManageRoiSettings ? '<button type="button" class="button primary consultant-roi-settings-apply" id="consultant-roi-settings-apply" disabled>应用于全员</button>' : '';
    page.innerHTML = `<div class="page-title"><div><h1>顾问投入产出比</h1><p>${escapeHtml(data.company)} · ${escapeHtml(data.period)} · 按顾问部门归集</p></div><div class="consultant-roi-page-actions">${filterHtml()}<div class="consultant-roi-refresh-control"><button class="button consultant-roi-refresh" id="consultant-roi-refresh" type="button"><span aria-hidden="true">↻</span>刷新数据</button><small id="consultant-roi-refresh-status" aria-live="polite">${escapeHtml(refreshedAt)} 已更新 · 每 60 秒自动检查新发布版本</small></div></div></div><div class="analysis-layout-grid consultant-roi-layout"><section class="panel analysis-source" data-analysis-block="consultant_roi_source"><strong>数据来源</strong><span>工资：${escapeHtml(data.sources.payroll?.fileName || '未上传')} / ${escapeHtml(data.sources.payrollSheet || '—')}；产出：${escapeHtml(data.sources.revenue?.fileName || '未上传')} / ${escapeHtml(data.sources.revenueSheet || '—')}；投流：${escapeHtml(data.sources.spend?.fileName || '未上传')} / ${escapeHtml(data.sources.spendSheet || '—')}；${escapeHtml(directoryText)}</span><small>${payrollFieldText ? `工资取数字段：${escapeHtml(payrollFieldText)}。` : ''}${revenueFieldText ? `营收取数字段：${escapeHtml(revenueFieldText)}，仅取 ${escapeHtml(data.period)}。` : ''}${spendFieldText ? `投流取数字段：${escapeHtml(spendFieldText)}。` : ''}${departmentText ? escapeHtml(departmentText) : ''}${data.canViewMatchDiagnostics && Number(data.sources.unmatchedRevenueRows || 0) ? `另有 ${Number(data.sources.unmatchedRevenueRows)} 条营收明细因人员不在顾问部门名册中未纳入。` : ''}${escapeHtml(spendIssueText)}${staleReimbursementText ? `<span class="consultant-roi-setting-warning">${escapeHtml(staleReimbursementText)}</span>` : ''}报销费用按管理员为本期间保存的序时账二级科目计入；入职日期只取工资表；英文名优先取企业微信通讯录，所属公司、离职状态和离职日期取花名册受控列。投流消耗只按英文名唯一匹配；管理员保存的投入标签与报销科目对全员生效。${sourceState}</small></section><section id="consultant-roi-metrics" class="card-grid consultant-roi-metrics" data-analysis-block="consultant_roi_metrics"></section><section class="panel consultant-roi-table-panel" data-analysis-block="consultant_roi_table"><div class="consultant-roi-table-toolbar"><div><h2>顾问投入产出明细</h2><div class="panel-sub"><span id="consultant-roi-count"></span> · 点击表头漏斗进行筛选和排序</div></div><div class="consultant-roi-actions"><div class="consultant-roi-input-group"><small>计入投入</small><div class="consultant-roi-tags" role="group" aria-label="当前计入投入合计的数据项目">${inputTags}</div></div><div class="consultant-roi-utility-actions">${settingsApplyButton}${visibilityButton}<button type="button" class="button" id="consultant-roi-clear">清除筛选</button></div></div></div><div class="table-wrap consultant-roi-table-wrap" role="region" aria-label="顾问投入产出明细" tabindex="0"><table class="data-table consultant-roi-table"><thead id="consultant-roi-head"></thead><tbody id="consultant-roi-body"></tbody></table></div></section></div><div id="consultant-roi-filter-menu" class="roi-filter-menu hidden" role="dialog" aria-modal="false"></div>${data.canManageRoiSettings ? '<div id="consultant-roi-reimbursement-menu" class="consultant-roi-reimbursement-menu hidden" role="dialog" aria-label="配置报销费用二级科目"></div>' : ''}<div id="consultant-personnel-popover" class="consultant-personnel-popover hidden" role="tooltip"></div>`;
    page.insertAdjacentHTML('beforeend', `<div id="consultant-roi-average-modal" class="consultant-roi-average-modal hidden" role="dialog" aria-modal="true" aria-labelledby="consultant-roi-average-title"><section class="consultant-roi-average-dialog"><button type="button" class="consultant-roi-average-close" aria-label="关闭平均投入产出比">×</button><header><span>AVERAGE ROI</span><h2 id="consultant-roi-average-title">平均投入产出比</h2><p>所有顾问与各所属公司</p></header><div id="consultant-roi-average-body"></div></section></div>`);
    page.insertAdjacentHTML('beforeend', `<div id="consultant-roi-drilldown-modal" class="consultant-roi-drilldown-modal hidden" role="dialog" aria-modal="true" aria-labelledby="consultant-roi-drilldown-title"><section class="consultant-roi-drilldown-dialog"><button type="button" class="consultant-roi-drilldown-close" aria-label="关闭顾问分析弹窗">×</button><header><span id="consultant-roi-drilldown-kicker">CONSULTANT</span><h2 id="consultant-roi-drilldown-title">顾问分析</h2><p id="consultant-roi-drilldown-sub"></p></header><div id="consultant-roi-drilldown-body" class="consultant-roi-drilldown-body"></div></section></div>`);
    if (data.canManageVisibility) page.insertAdjacentHTML('beforeend', `<div id="consultant-roi-visibility-modal" class="consultant-roi-visibility-modal hidden" role="dialog" aria-modal="true" aria-labelledby="consultant-roi-visibility-title"><section><button type="button" class="consultant-roi-visibility-close" aria-label="关闭名单显示设置">×</button><header><span>ADMIN ONLY</span><h2 id="consultant-roi-visibility-title">名单显示设置</h2><p>取消勾选的顾问不会出现在明细、指标卡和所属公司汇总中。</p></header><div class="consultant-roi-visibility-tools"><input id="consultant-roi-visibility-search" type="search" placeholder="搜索顾问姓名" aria-label="搜索顾问姓名"><button type="button" class="button" id="consultant-roi-show-all">全部显示</button></div><div id="consultant-roi-visibility-list" class="consultant-roi-visibility-list"></div><footer><span id="consultant-roi-visibility-count"></span><div><button type="button" class="button" id="consultant-roi-visibility-cancel">取消</button><button type="button" class="button primary" id="consultant-roi-visibility-save">保存设置</button></div></footer></section></div>`);
    const sourcePanel = page.querySelector('[data-analysis-block="consultant_roi_source"]');
    if (directoryGuide) sourcePanel?.insertAdjacentHTML('beforeend', consultantDirectoryGuideHtml(directorySync, directoryGuide));
    page.insertAdjacentHTML('beforeend', `<div id="consultant-directory-guide-modal" class="consultant-directory-guide-modal hidden" role="dialog" aria-modal="true" aria-labelledby="consultant-directory-guide-title"><section><button type="button" class="consultant-directory-guide-close" aria-label="关闭处理指引">×</button><span class="consultant-directory-guide-kicker">WECOM SYNC</span><h2 id="consultant-directory-guide-title">${escapeHtml(directoryGuide?.title || '企业微信匹配指引')}</h2><p>${escapeHtml(directorySync.message || '')}</p><ol>${(directoryGuide?.steps || []).map(step => `<li>${escapeHtml(step)}</li>`).join('')}</ol><footer><small>${directorySync.updatedAt ? `状态更新：${escapeHtml(directorySync.updatedAt)}` : '服务器尚未写入同步状态'}${authExpiresText ? ` · 链接约 ${escapeHtml(authExpiresText)} 失效` : ''}</small><div class="consultant-directory-auth-actions">${authorizationActions}</div></footer></section></div>`);
    const refreshButton = $('#consultant-roi-refresh'); if (refreshButton) refreshButton.innerHTML = '<span aria-hidden="true">↻</span>重新匹配';
    const refreshStatus = $('#consultant-roi-refresh-status'); if (refreshStatus) refreshStatus.textContent = ['pending', 'running'].includes(directorySync.state) || consultantDirectoryPollingUntil > Date.now() ? `${refreshedAt} 已更新 · 每 3 秒检查匹配或授权进度` : `${refreshedAt} 已更新 · 发布工资表/营收统计表/顾问消耗表后自动刷新`;
    ...
    const filterMenu = $('#consultant-roi-filter-menu');
    const reimbursementMenu = $('#consultant-roi-reimbursement-menu'); const reimbursementButton = $('#consultant-roi-reimbursement-open');
    const closeColumnFilter = () => {
      filterMenu.classList.add('hidden'); filterMenu.replaceChildren();
      page.querySelectorAll('[data-roi-filter-toggle]').forEach(button => button.setAttribute('aria-expanded', 'false'));
      window.removeEventListener('scroll', closeColumnFilter);
    };
    const selectedReimbursementNames = () => new Set(consultantRoiView.reimbursementAccounts);
    const closeReimbursementMenu = () => { reimbursementMenu?.classList.add('hidden'); reimbursementButton?.setAttribute('aria-expanded', 'false'); window.removeEventListener('scroll', closeReimbursementMenu); };
    const positionReimbursementMenu = () => {
      if (!reimbursementMenu || !reimbursementButton) return;
      const rect = reimbursementButton.getBoundingClientRect(); const gap = 7; const edge = 12;
      const left = Math.max(edge, Math.min(rect.right - reimbursementMenu.offsetWidth, window.innerWidth - reimbursementMenu.offsetWidth - edge));
      const below = rect.bottom + gap; const top = below + reimbursementMenu.offsetHeight <= window.innerHeight - edge ? below : Math.max(edge, rect.top - reimbursementMenu.offsetHeight - gap);
      reimbursementMenu.style.left = `${left}px`; reimbursementMenu.style.top = `${top}px`;
    };
    const renderReimbursementMenu = () => {
      if (!reimbursementMenu) return;
      const selectedNames = selectedReimbursementNames();
      const keyword = String(reimbursementMenu.dataset.search || '').trim().toLocaleLowerCase('zh-CN'); const visibleOptions = reimbursementOptions.filter(item => !keyword || item.name.toLocaleLowerCase('zh-CN').includes(keyword));
      reimbursementMenu.innerHTML = `<header><div><strong>报销费用科目设置</strong><small>${escapeHtml(data.period)} · 保存后应用于全员</small></div><button type="button" data-roi-reimbursement-close aria-label="关闭报销费用科目设置">×</button></header>${reimbursementOptions.length > 6 ? `<label class="consultant-roi-reimbursement-search"><span class="sr-only">搜索二级科目</span><input type="search" value="${escapeHtml(reimbursementMenu.dataset.search || '')}" placeholder="搜索二级科目" aria-label="搜索报销费用二级科目"></label>` : ''}<div class="consultant-roi-reimbursement-actions"><button type="button" data-roi-reimbursement-all>全选</button><button type="button" data-roi-reimbursement-none>清空</button></div><div class="consultant-roi-reimbursement-options">${visibleOptions.map(item => `<label title="${escapeHtml(item.name)}"><input type="checkbox" data-roi-reimbursement-account="${escapeHtml(item.name)}" ${selectedNames.has(item.name) ? 'checked' : ''}><span>${escapeHtml(item.name)}</span><small><b>${money(item.amount)}</b><i>${Number(item.detailCount || 0)} 笔</i></small></label>`).join('') || '<div class="empty">没有匹配的报销费用科目</div>'}</div><footer><span>已选择 ${selectedNames.size} / ${reimbursementNames.length} 项</span><div><button type="button" class="button" data-roi-reimbursement-cancel>取消</button><button type="button" class="button primary" data-roi-reimbursement-save ${consultantRoiView.settingsDirty ? '' : 'disabled'}>保存并应用</button></div></footer>`;
      reimbursementMenu.querySelector('[data-roi-reimbursement-close]').onclick = closeReimbursementMenu;
      reimbursementMenu.querySelector('[data-roi-reimbursement-cancel]').onclick = () => { consultantRoiView.reimbursementAccounts = new Set(consultantRoiView.savedReimbursementAccounts); consultantRoiView.settingsDirty = consultantRoiSettingsAreDirty(); closeReimbursementMenu(); renderView(); };
      reimbursementMenu.querySelector('[data-roi-reimbursement-all]').onclick = () => { consultantRoiView.reimbursementAccounts = new Set(reimbursementNames); consultantRoiView.settingsDirty = consultantRoiSettingsAreDirty(); renderView(); };
      reimbursementMenu.querySelector('[data-roi-reimbursement-none]').onclick = () => { consultantRoiView.reimbursementAccounts = new Set(); consultantRoiView.settingsDirty = consultantRoiSettingsAreDirty(); renderView(); };
      reimbursementMenu.querySelectorAll('[data-roi-reimbursement-account]').forEach(input => input.onchange = () => {
        const next = selectedReimbursementNames(); if (input.checked) next.add(input.dataset.roiReimbursementAccount); else next.delete(input.dataset.roiReimbursementAccount);
        consultantRoiView.reimbursementAccounts = next; consultantRoiView.settingsDirty = consultantRoiSettingsAreDirty(); renderView();
      });
      const search = reimbursementMenu.querySelector('input[type="search"]'); if (search) search.oninput = () => { reimbursementMenu.dataset.search = search.value; renderReimbursementMenu(); reimbursementMenu.querySelector('input[type="search"]')?.focus(); positionReimbursementMenu(); };
      reimbursementMenu.querySelector('[data-roi-reimbursement-save]').onclick = () => persistRoiSettings(reimbursementMenu.querySelector('[data-roi-reimbursement-save]'));
    };
    const openReimbursementMenu = () => {
      if (!reimbursementMenu || !reimbursementButton) return;
      const willOpen = reimbursementMenu.classList.contains('hidden'); closeColumnFilter(); closeReimbursementMenu();
      if (!willOpen) return; reimbursementMenu.dataset.search = ''; renderReimbursementMenu(); reimbursementMenu.classList.remove('hidden'); reimbursementButton.setAttribute('aria-expanded', 'true'); positionReimbursementMenu(); window.addEventListener('scroll', closeReimbursementMenu, { passive: true });
    };
    const positionColumnFilter = button => {
      const rect = button.getBoundingClientRect(); const gap = 7; const edge = 12;
      const contentEdge = page.getBoundingClientRect().left + 8;
      const left = Math.max(edge, contentEdge, Math.min(rect.right - filterMenu.offsetWidth, window.innerWidth - filterMenu.offsetWidth - edge));
      const below = rect.bottom + gap; const top = below + filterMenu.offsetHeight <= window.innerHeight - edge ? below : Math.max(edge, rect.top - filterMenu.offsetHeight - gap);
      filterMenu.style.left = `${left}px`; filterMenu.style.top = `${top}px`;
    };
    const openColumnFilter = (column, button) => {
      const isSameOpen = !filterMenu.classList.contains('hidden') && filterMenu.dataset.roiFilterMenu === column.key;
      closeReimbursementMenu(); closeColumnFilter(); if (isSameOpen) return;
      const activeSort = consultantRoiView.sortKey === column.key ? consultantRoiView.sortDirection : '';
      filterMenu.dataset.roiFilterMenu = column.key; filterMenu.setAttribute('aria-label', `${column.label}筛选与排序`);
      filterMenu.innerHTML = `<header><strong>${escapeHtml(column.label)}</strong><button type="button" data-roi-filter-close aria-label="关闭筛选">×</button></header><div class="roi-filter-sort-actions"><button type="button" data-roi-menu-sort="asc" class="${activeSort === 'asc' ? 'active' : ''}"><span>↑</span>${column.type === 'number' ? '从小到大' : '升序排列'}</button><button type="button" data-roi-menu-sort="desc" class="${activeSort === 'desc' ? 'active' : ''}"><span>↓</span>${column.type === 'number' ? '从大到小' : '降序排列'}</button></div><label><span>筛选条件</span>${consultantRoiFilterControlHtml(column)}</label><footer><button type="button" class="button" data-roi-filter-clear>清除此列</button><button type="button" class="button primary" data-roi-filter-apply>应用</button></footer>`;
      filterMenu.classList.remove('hidden'); button.setAttribute('aria-expanded', 'true'); positionColumnFilter(button); window.addEventListener('scroll', closeColumnFilter, { passive: true });
      const draft = filterMenu.querySelector('[data-roi-filter-draft]'); draft?.focus(); if (draft?.select) draft.select();
      filterMenu.querySelector('[data-roi-filter-close]').onclick = closeColumnFilter;
      filterMenu.querySelectorAll('[data-roi-menu-sort]').forEach(sortButton => sortButton.onclick = () => { consultantRoiView.sortKey = column.key; consultantRoiView.sortDirection = sortButton.dataset.roiMenuSort; closeColumnFilter(); renderView(); });
      const apply = () => { const value = String(draft?.value || '').trim(); if (value) consultantRoiView.filters[column.key] = value; else delete consultantRoiView.filters[column.key]; closeColumnFilter(); renderView(); };
      filterMenu.querySelector('[data-roi-filter-apply]').onclick = apply;
      filterMenu.querySelector('[data-roi-filter-clear]').onclick = () => { delete consultantRoiView.filters[column.key]; closeColumnFilter(); renderView(); };
      draft.onkeydown = event => { if (event.key === 'Enter') apply(); if (event.key === 'Escape') closeColumnFilter(); };
    };
    const persistRoiSettings = async button => {
      if (!data.canManageRoiSettings || !consultantRoiView.settingsDirty || button?.disabled) return;
      const originalText = button?.textContent || '保存并应用'; if (button) { button.disabled = true; button.textContent = '保存中…'; }
      try {
        await api('/api/analysis/consultant-roi/settings', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ period: data.period, inputs: consultantRoiView.inputs, reimbursementAccounts: [...consultantRoiView.reimbursementAccounts] }) });
        closeReimbursementMenu(); consultantRoiSourceRevision = ''; showNotice('投入项目与报销科目已应用于全员'); await renderConsultantRoiInteractive({ trigger: 'manual' });
      } catch (error) { showNotice(error.message, true); if (button?.isConnected) { button.disabled = false; button.textContent = originalText; } }
    };
    const drilldownModal = $('#consultant-roi-drilldown-modal'); const drilldownBody = $('#consultant-roi-drilldown-body'); let drilldownRequest = 0; let drilldownReturnFocus = null;
    const closeDrilldown = () => { drilldownRequest += 1; drilldownModal.classList.add('hidden'); drilldownReturnFocus?.focus?.(); drilldownReturnFocus = null; };
    const prepareDrilldown = (button, kicker, title, sub) => {
      drilldownReturnFocus = button; $('#consultant-roi-drilldown-kicker').textContent = kicker; $('#consultant-roi-drilldown-title').textContent = title; $('#consultant-roi-drilldown-sub').textContent = sub;
      drilldownBody.innerHTML = '<div class="empty consultant-roi-drilldown-loading">正在读取数据…</div>'; drilldownModal.classList.remove('hidden'); page.querySelector('.consultant-roi-drilldown-close').focus(); return ++drilldownRequest;
    };
    const openConsultantTrend = async button => {
      const consultant = button.dataset.consultantTrend; const request = prepareDrilldown(button, 'MONTHLY TREND', `${consultant} · 年度趋势`, `${data.period.slice(0, 4)} 年预计营收与投入产出比`);
      try {
        const result = await api(`/api/analysis/consultant-roi/trend?company=${encodeURIComponent(scope.company)}&period=${encodeURIComponent(scope.period)}&year=${encodeURIComponent(data.period.slice(0, 4))}&consultant=${encodeURIComponent(consultant)}`, { cache: 'no-store' });
        if (request !== drilldownRequest || !drilldownBody.isConnected) return; drilldownBody.innerHTML = consultantRoiTrendChartHtml(result.points || []);
      } catch (error) { if (request === drilldownRequest && drilldownBody.isConnected) drilldownBody.innerHTML = `<div class="empty">${escapeHtml(error.message)}</div>`; }
    };
    const openConsultantRevenue = async button => {
      const consultant = button.dataset.consultantRevenue; const request = prepareDrilldown(button, 'REVENUE DETAILS', `${consultant} · 营收明细`, `${data.period} · 合同及客户构成`);
      try {
        const result = await api(`/api/analysis/consultant-roi/revenue-details?company=${encodeURIComponent(scope.company)}&period=${encodeURIComponent(scope.period)}&consultant=${encodeURIComponent(consultant)}`, { cache: 'no-store' });
        if (request !== drilldownRequest || !drilldownBody.isConnected) return; drilldownBody.innerHTML = consultantRevenueDetailsHtml(result);
      } catch (error) { if (request === drilldownRequest && drilldownBody.isConnected) drilldownBody.innerHTML = `<div class="empty">${escapeHtml(error.message)}</div>`; }
    };
    page.querySelector('.consultant-roi-drilldown-close').onclick = closeDrilldown; drilldownModal.onclick = event => { if (event.target === drilldownModal) closeDrilldown(); }; drilldownModal.onkeydown = event => { if (event.key === 'Escape') closeDrilldown(); };
    let closePersonnelPopover = () => {};
    const renderView = () => {
      closePersonnelPopover();
      ...
      const visibleColumns = consultantRoiColumns.filter(column => consultantRoiView.inputs[column.key] !== false && (!column.adminOnly || data.canViewMatchDiagnostics));
      metricCards.innerHTML = `<article class="card"><div class="metric-label">投入合计</div><div class="metric-value">${money(totals.input)}</div><div class="metric-note">当前口径与筛选范围</div></article><article class="card"><div class="metric-label">预计营收</div><div class="metric-value positive">${money(totals.output)}</div><div class="metric-note">当前筛选结果汇总</div></article><article class="card"><div class="metric-label">整体投入产出比</div><div class="metric-value">${roi == null ? '—' : `${roi.toFixed(2)} 倍`}</div><div class="metric-note">预计营收 ÷ 已选投入</div></article><button type="button" class="card consultant-roi-average-card" id="consultant-roi-average-open"><div class="metric-label">顾问平均投入产出比</div><div class="metric-value">${average.averageRoi == null ? '—' : `${average.averageRoi.toFixed(2)} 倍`}</div><div class="metric-note">点击查看所有顾问及各公司</div></button><article class="card"><div class="metric-label">顾问人数</div><div class="metric-value">${rows.length}</div><div class="metric-note">工资表顾问部门名册</div></article>`;
      $('#consultant-roi-average-body').innerHTML = `<div class="consultant-roi-average-overall"><span>所有顾问平均投入产出比</span><strong>${average.averageRoi == null ? '—' : `${average.averageRoi.toFixed(2)} 倍`}</strong><small>${average.consultantCount} 名有投入顾问的个人投入产出比算术平均</small></div><div class="consultant-roi-region-list">${average.companies.map(item => `<article><div><strong>${escapeHtml(item.companyName)}</strong><small>${item.consultantCount} 名顾问</small></div><b>${item.roi == null ? '—' : `${item.roi.toFixed(2)} 倍`}</b><span>投入 ${money(item.input)} · 预计营收 ${money(item.output)}</span></article>`).join('') || '<div class="empty">暂无可汇总的所属公司</div>'}</div><p class="consultant-roi-average-note">各公司投入产出比按企业微信花名册“所属公司”归集；未维护公司的人显示为“待补充”。</p>`;
      $('#consultant-roi-count').textContent = `共 ${rawRows.length} 人，当前显示 ${rows.length} 人`;
      page.querySelectorAll('[data-roi-input]').forEach(button => { const selected = consultantRoiView.inputs[button.dataset.roiInput]; const label = consultantRoiInputDefinitions.find(item => item.key === button.dataset.roiInput)?.label || ''; const accountCount = button.dataset.roiInput === 'journalExpense' ? `<b>${selectedReimbursementNames().size}/${reimbursementNames.length}</b>` : ''; button.className = `consultant-roi-tag ${selected ? 'selected' : ''}`; button.setAttribute('aria-pressed', String(selected)); button.innerHTML = `<span>${selected ? '✓' : '+'}</span>${label}${accountCount}`; button.onclick = () => { consultantRoiView.inputs[button.dataset.roiInput] = !selected; consultantRoiView.settingsDirty = consultantRoiSettingsAreDirty(); if (selected) { delete consultantRoiView.filters[button.dataset.roiInput]; if (consultantRoiView.sortKey === button.dataset.roiInput) { consultantRoiView.sortKey = 'input'; consultantRoiView.sortDirection = 'desc'; } } renderView(); }; });
      const menuOpen = Boolean(reimbursementMenu && !reimbursementMenu.classList.contains('hidden')); renderReimbursementMenu(); if (menuOpen) positionReimbursementMenu();
      if (reimbursementButton) { reimbursementButton.classList.toggle('dirty', consultantRoiView.settingsDirty); reimbursementButton.onclick = event => { event.stopPropagation(); openReimbursementMenu(); }; }
      const settingsApply = $('#consultant-roi-settings-apply'); if (settingsApply) { settingsApply.disabled = !consultantRoiView.settingsDirty; settingsApply.classList.toggle('dirty', consultantRoiView.settingsDirty); settingsApply.onclick = () => persistRoiSettings(settingsApply); }
      $('.consultant-roi-table').querySelector('colgroup')?.remove(); $('.consultant-roi-table').insertAdjacentHTML('afterbegin', `<colgroup>${visibleColumns.map(column => `<col class="roi-col-${column.key}">`).join('')}</colgroup>`);
      $('#consultant-roi-head').innerHTML = `<tr>${visibleColumns.map(column => { const active = consultantRoiView.sortKey === column.key; const filtered = Boolean(consultantRoiView.filters[column.key]); return `<th class="roi-head-${column.type} roi-head-${column.key}"><div class="roi-column-head"><button type="button" class="roi-sort ${active ? 'active' : ''}" data-roi-sort="${column.key}" aria-label="${column.label}，点击排序"><span class="roi-sort-label">${column.label}</span>${active ? `<span class="roi-sort-direction" aria-hidden="true">${consultantRoiView.sortDirection === 'asc' ? '↑' : '↓'}</span>` : ''}</button><button type="button" class="roi-filter-trigger ${filtered ? 'active' : ''}" data-roi-filter-toggle="${column.key}" aria-label="筛选${column.label}${filtered ? '，已应用筛选' : ''}" aria-expanded="false" aria-controls="consultant-roi-filter-menu">${consultantRoiFilterIcon}${filtered ? '<i aria-hidden="true"></i>' : ''}</button></div></th>`; }).join('')}</tr>`;
      ...
      $('#consultant-roi-body').innerHTML = rows.map(row => `<tr><td class="roi-cell-name"><span class="consultant-name-cell"><button type="button" class="consultant-trend-open" data-consultant-trend="${escapeHtml(row.name)}" aria-label="查看${escapeHtml(row.name)}的营收和投入产出比趋势">${escapeHtml(row.name)}</button>${row.isNewEmployee ? statusBadge(row, 'new') : ''}${row.isResigned ? statusBadge(row, 'resigned') : ''}</span></td><td class="roi-cell-english" title="${escapeHtml(row.englishName || '')}">${escapeHtml(row.englishName || '—')}</td><td class="roi-cell-date">${escapeHtml(row.hireDate || '—')}</td><td class="roi-cell-company" title="${escapeHtml(row.companyName || '待补充')}">${escapeHtml(row.companyName || '待补充')}</td>${optionalAmountCell(row, 'baseSalary')}${optionalAmountCell(row, 'commission')}${optionalAmountCell(row, 'journalExpense')}${optionalAmountCell(row, 'trafficSpend')}<td class="num">${money(row.input)}</td><td class="num">${Number(row.output || 0) ? `<button type="button" class="consultant-revenue-open" data-consultant-revenue="${escapeHtml(row.name)}" aria-label="查看${escapeHtml(row.name)}的本月营收明细">${money(row.output)}</button>` : money(row.output)}</td><td class="num roi-value">${row.roi == null ? '—' : `${row.roi.toFixed(2)} 倍`}</td>${statusCell(row)}</tr>`).join('') || `<tr><td colspan="${visibleColumns.length}" class="empty">当前口径或筛选条件下暂无顾问数据</td></tr>`;
      ...
      const showPersonnelPopover = (button, pinned = false) => {
        const rect = button.getBoundingClientRect(); const edge = 10; const gap = 8; personnelPopover.textContent = `${button.dataset.consultantStatusTitle}：${button.dataset.consultantStatusDate || '待补充'}`; personnelPopover.classList.remove('hidden');
        const left = Math.max(edge, Math.min(rect.left + rect.width / 2 - personnelPopover.offsetWidth / 2, window.innerWidth - personnelPopover.offsetWidth - edge));
        const below = rect.bottom + gap; const top = below + personnelPopover.offsetHeight <= window.innerHeight - edge ? below : Math.max(edge, rect.top - personnelPopover.offsetHeight - gap);
        personnelPopover.style.left = `${left}px`; personnelPopover.style.top = `${top}px`; button.setAttribute('aria-expanded', 'true'); if (pinned) pinnedBadge = button; window.addEventListener('scroll', closePersonnelPopover, { passive: true });
      };
      page.querySelectorAll('[data-consultant-status-date]').forEach(button => {
        button.setAttribute('aria-expanded', 'false'); button.onmouseenter = () => showPersonnelPopover(button); button.onmouseleave = () => { if (pinnedBadge !== button) closePersonnelPopover(); };
        button.onfocus = () => showPersonnelPopover(button); button.onblur = () => { if (pinnedBadge !== button) closePersonnelPopover(); };
        button.onclick = event => { event.stopPropagation(); if (pinnedBadge === button) closePersonnelPopover(); else { closePersonnelPopover(); showPersonnelPopover(button, true); } };
        button.onkeydown = event => { if (event.key === 'Escape') { closePersonnelPopover(); button.focus(); } };
      });
      page.querySelectorAll('[data-consultant-trend]').forEach(button => button.onclick = event => { event.stopPropagation(); openConsultantTrend(button); });
      page.querySelectorAll('[data-consultant-revenue]').forEach(button => button.onclick = event => { event.stopPropagation(); openConsultantRevenue(button); });
      page.querySelectorAll('[data-roi-sort]').forEach(button => button.onclick = () => { const key = button.dataset.roiSort; if (consultantRoiView.sortKey === key) consultantRoiView.sortDirection = consultantRoiView.sortDirection === 'asc' ? 'desc' : 'asc'; else { consultantRoiView.sortKey = key; consultantRoiView.sortDirection = consultantRoiColumns.find(column => column.key === key)?.type === 'number' ? 'desc' : 'asc'; } closeColumnFilter(); renderView(); });
      page.querySelectorAll('[data-roi-filter-toggle]').forEach(button => button.onclick = event => { event.stopPropagation(); const column = visibleColumns.find(item => item.key === button.dataset.roiFilterToggle); if (column) openColumnFilter(column, button); });
    ...
    if (data.canManageVisibility) {
      const modal = $('#consultant-roi-visibility-modal'); const list = $('#consultant-roi-visibility-list'); const search = $('#consultant-roi-visibility-search'); const options = data.visibility?.options || []; const hidden = new Set(options.filter(item => item.hidden).map(item => item.canonicalName));
      const closeVisibility = () => modal.classList.add('hidden');
      const renderVisibilityList = () => {
        const keyword = String(search.value || '').trim().toLocaleLowerCase('zh-CN'); const visible = options.filter(item => !keyword || String(item.name || '').toLocaleLowerCase('zh-CN').includes(keyword));
        list.innerHTML = visible.map(item => `<label><input type="checkbox" data-consultant-visibility="${escapeHtml(item.canonicalName)}" ${hidden.has(item.canonicalName) ? '' : 'checked'}><span>${escapeHtml(item.name)}</span>${hidden.has(item.canonicalName) ? '<small>已隐藏</small>' : ''}</label>`).join('') || '<div class="empty">没有匹配的顾问</div>';
        $('#consultant-roi-visibility-count').textContent = `已隐藏 ${hidden.size} 人`;
        list.querySelectorAll('[data-consultant-visibility]').forEach(input => input.onchange = () => { if (input.checked) hidden.delete(input.dataset.consultantVisibility); else hidden.add(input.dataset.consultantVisibility); renderVisibilityList(); });
      };
      $('#consultant-roi-visibility-open').onclick = () => { search.value = ''; renderVisibilityList(); modal.classList.remove('hidden'); search.focus(); };
      page.querySelector('.consultant-roi-visibility-close').onclick = closeVisibility; $('#consultant-roi-visibility-cancel').onclick = closeVisibility; modal.onclick = event => { if (event.target === modal) closeVisibility(); };
      search.oninput = renderVisibilityList; $('#consultant-roi-show-all').onclick = () => { hidden.clear(); renderVisibilityList(); };
      $('#consultant-roi-visibility-save').onclick = async () => {
        const button = $('#consultant-roi-visibility-save'); button.disabled = true; button.textContent = '保存中…';
        try {
          await api('/api/analysis/consultant-roi/visibility', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ hiddenConsultants: [...hidden] }) });
          closeVisibility(); consultantRoiSourceRevision = ''; showNotice('顾问名单显示设置已保存并对所有查看人员生效'); await renderConsultantRoiInteractive({ trigger: 'manual' });
        } catch (error) { showNotice(error.message, true); if (button.isConnected) { button.disabled = false; button.textContent = '保存设置'; } }
      };
    }
    page.onclick = event => { if (!event.target.closest('#consultant-roi-filter-menu') && !event.target.closest('[data-roi-filter-toggle]')) closeColumnFilter(); if (!event.target.closest('#consultant-roi-reimbursement-menu') && !event.target.closest('#consultant-roi-reimbursement-open')) closeReimbursementMenu(); if (!event.target.closest('[data-consultant-status-date]')) closePersonnelPopover(); };
    page.querySelector('.consultant-roi-table-wrap').onscroll = () => { closeColumnFilter(); closeReimbursementMenu(); closePersonnelPopover(); };
    const guideModal = $('#consultant-directory-guide-modal'); const openDirectoryGuide = () => guideModal.classList.remove('hidden'); const closeDirectoryGuide = () => guideModal.classList.add('hidden');
    $('#consultant-directory-guide-open')?.addEventListener('click', openDirectoryGuide); page.querySelector('.consultant-directory-guide-close').onclick = closeDirectoryGuide; guideModal.onclick = event => { if (event.target === guideModal) closeDirectoryGuide(); };
    $('#consultant-directory-guide-retry')?.addEventListener('click', () => { closeDirectoryGuide(); requestConsultantDirectoryRefresh(); });
    $('#consultant-directory-auth-request')?.addEventListener('click', requestConsultantDirectoryAuthorization);
    $('#consultant-directory-auth-link')?.addEventListener('click', () => { consultantDirectoryPollingUntil = Date.now() + 16 * 60_000; scheduleConsultantRoiAutoRefresh(); });
    renderView(); bindCommonFilters(); $('#consultant-roi-refresh').onclick = requestConsultantDirectoryRefresh; applyAnalysisBlockLayout(page.querySelector('.consultant-roi-layout'), consultantRoiModuleKey);
    const guideKey = `${directorySync.state || ''}:${directorySync.updatedAt || ''}:${directorySync.authUrlExpiresAt || ''}`; if (directoryGuide && ['warning', 'error'].includes(directoryGuide.tone) && consultantDirectoryGuideSeenKey !== guideKey) { consultantDirectoryGuideSeenKey = guideKey; openDirectoryGuide(); }
  } catch (error) {
    if (!isCurrent()) return;
...
async function renderIntercompanyReconciliation() {
  ...
  try {
    ...
    const scopeWarning = !data.scopeComplete ? `<div class="intercompany-scope-warning"><strong>当前授权范围不足以形成集团完整校验</strong><span>缺少：${escapeHtml((data.missingScopeCompanies || []).join('、') || '未授权公司')}；仅展示员工有权访问的公司组合。</span></div>` : '';
...
const statementMeta = (raw, title, data, companyOverride = '') => {
  ...
  const company = cellText(companyOverride || data.company || companyCell).replace(/^编制单位\s*[:：]?\s*/, '') || data.company || '';
...
function renderJournalStatement(data) {
  ...
  const columns = journalDisplayColumns(raw);
  const body = rows.map(row => { const item = journalDisplayRow(row, columns); return `<tr><td>${escapeHtml(item.date)}</td><td>${escapeHtml(item.voucher)}</td><td class="journal-summary-text">${escapeHtml(item.summary)}</td><td>${escapeHtml(item.accountCode)}</td><td class="journal-account-text">${escapeHtml(item.account)}</td><td class="amount">${escapeHtml(statementAmount(item.debit))}</td><td class="amount">${escapeHtml(statementAmount(item.credit))}</td></tr>`; }).join('');
...
const revenueCumulativeTabLabelHtml = title => {
  const text = String(title || '').trim();
  const match = text.match(/^(.*?)[（(]([^）)]+)[）)]\s*$/);
  if (!match) return `<span class="revenue-tab-label"><i>${escapeHtml(text)}</i></span>`;
  return `<span class="revenue-tab-label"><i>${escapeHtml(match[1].trim())}</i><small>（${escapeHtml(match[2].trim())}）</small></span>`;
};
const revenueCumulativePanelHtml = (cumulativeYears, cumulativeIssues) => {
  if (!cumulativeYears.length) {
    ...
    return `<section class="panel revenue-cumulative-panel"><div class="revenue-cumulative-title"><div><span>年度经营累计</span><h2>年度累计明细</h2></div></div><div class="revenue-cumulative-empty"><strong>暂无累计数据</strong>${issue}</div></section>`;
  }
...
async function renderRevenueStatistics() {
  ...
  try {
    ...
    if (!dimensions.length && state.revenueDimension !== 'cumulative') {
    ...
    if (state.revenueDimension !== 'cumulative' && !dimensions.some(item => item.key === state.revenueDimension)) state.revenueDimension = dimensions[0]?.key || 'group';
    const paint = () => {
      const sourceState = data.meta?.status === 'published' ? '当前发布' : data.meta?.status === 'validated' ? '待发布预览' : data.meta?.status || '原始资料';
      if (state.revenueDimension === 'cumulative') {
        page.innerHTML = `<div class="page-title revenue-page-title"><div><h1>营收统计累计数据</h1><p>${escapeHtml(data.company)} · ${escapeHtml(data.period)} · 年度累计子表独立查看</p></div><div class="revenue-source-badge"><span>${escapeHtml(sourceState)}</span><strong>${escapeHtml(data.meta?.fileName || data.raw?.sourceSheet || '营收统计汇总表')}</strong></div></div>${revenueCumulativePanelHtml(cumulativeYears, cumulativeIssues)}`;
...
async function renderUploads() {
  ...
  const setUploadCompany = (value, { confirmed = false } = {}) => { const company = state.bootstrap.companies.find(item => item.key === value); if (!company) return false; const detectedKeys = [...new Set(Object.values(selected).filter(file => guessType(file?.webkitRelativePath || file?.name) !== quotationLedgerReportType).flatMap(file => guessCompanies(file?.webkitRelativePath || file?.name)))]; if (!confirmed && detectedKeys.length && (detectedKeys.length > 1 || detectedKeys[0] !== value)) { const detectedNames = detectedKeys.map(companyNameByKey).join('、'); if (!window.confirm(`已选择文件检测到的地区为“${detectedNames}”，当前准备选择“${company.name}”。\n\n地区不一致可能导致报表归属错误，确定仍选择 ${company.name}？`)) return false; } state.uploadCompany = value; $('#upload-company-select').value = value; $('#upload-company-value').textContent = company.name; companyPicker.querySelectorAll('[data-upload-company]').forEach(option => { const optionSelected = option.dataset.uploadCompany === value; option.classList.toggle('selected', optionSelected); option.setAttribute('aria-selected', String(optionSelected)); }); closeUploadPickers(); showSelectedScope(Object.values(selected)); return true; };
  const setUploadPeriod = (value, { confirmed = false } = {}) => { if (!/^\d{4}-\d{2}$/.test(value)) return false; const detectedPeriods = [...new Set(Object.values(selected).filter(file => guessType(file?.webkitRelativePath || file?.name) !== quotationLedgerReportType).map(file => guessPeriod(file?.webkitRelativePath || file?.name)).filter(Boolean))]; if (!confirmed && detectedPeriods.length && (detectedPeriods.length > 1 || detectedPeriods[0] !== value)) { if (!window.confirm(`已选择文件检测到的期间为“${detectedPeriods.join('、')}”，当前准备选择“${value}”。\n\n期间不一致可能导致报表归属错误，确定仍选择 ${value}？`)) return false; } state.uploadPeriod = value; $('#upload-period-select').value = value; const [year, month] = value.split('-'); $('#upload-period-value').textContent = `${year} 年 ${month} 月`; periodPicker.querySelectorAll('[data-upload-month]').forEach(option => option.classList.toggle('selected', option.dataset.uploadMonth === value)); closeUploadPickers(); showSelectedScope(Object.values(selected)); return true; };
  ...
  if (publishSelectedUploads) publishSelectedUploads.onclick = async () => {
    ...
    try { const result = await api('/api/uploads/bulk-publish', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ uploadKeys }) }); const refreshText = result.consultantDirectoryRefresh?.requested ? '；已自动刷新顾问人事匹配' : result.consultantDirectoryRefresh?.error ? `；${result.consultantDirectoryRefresh.error}` : ''; showNotice(`已批量发布 ${result.publishedCount} 个报表批次${refreshText}`); await renderUploads(); } catch (error) { showNotice(error.message, true); updateUploadSelection(); } finally { finishUploadHistoryMutation(); }
  };
  ...
  const selectedScope = files => { const list = [...files].filter(Boolean); const regular = list.filter(file => guessType(file?.webkitRelativePath || file?.name) !== quotationLedgerReportType); return { periods: [...new Set(regular.map(file => guessPeriod(file?.webkitRelativePath || file?.name)).filter(Boolean))], companies: [...new Set(regular.flatMap(file => guessCompanies(file?.webkitRelativePath || file?.name)))], quotationCount: list.length - regular.length }; };
  const showSelectedScope = files => { const list = [...files].filter(file => file && file.name); const { periods, companies, quotationCount } = selectedScope(list); const companyMismatch = companies.length > 1 || (companies.length === 1 && companies[0] !== state.uploadCompany); const periodMismatch = periods.length > 1 || (periods.length === 1 && periods[0] !== state.uploadPeriod); const fileList = $('#folder-file-list'); if (fileList) { const quotationFiles = list.filter(file => guessType(file?.webkitRelativePath || file?.name) === quotationLedgerReportType); const quotationScope = quotationCount ? `；${quotationCount} 份报单表归入桉侨集团全历史主数据，文件版本 ${quotationFiles.map(file => quotationVersion(file.name)).join('、')}` : ''; fileList.classList.toggle('scope-warning', companyMismatch || periodMismatch); fileList.textContent = list.length ? `${list.map(file => `${file.webkitRelativePath || file.name} → ${guessType(file.name) === 'bundle' ? '汇总财务报表（自动识别）' : reportNames[guessType(file.name)] || '未识别'}`).join('　')}　· 检测：${companies.length ? `地区 ${companies.map(companyNameByKey).join('、')}` : quotationCount === list.length ? '集团全历史主数据' : '未识别地区'}；${periods.length ? `期间 ${periods.join('、')}` : quotationCount === list.length ? '无需选择月份' : '未识别期间'}${quotationScope}${companyMismatch || periodMismatch ? `（当前选择：${companyNameByKey(state.uploadCompany)} / ${state.uploadPeriod}）` : ''}` : '尚未选择文件'; } return { companyMismatch, periodMismatch, companies, periods, quotationCount }; };
  ...
  uploadButton.onclick = async () => {
    ...
    for (const [reportType, file] of entries) {
      try {
        ...
        let entryPeriod = reportType === quotationLedgerReportType ? '' : period;
        ...
        const submit = () => api('/api/uploads', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ companyKey: entryCompanyKey, period: entryPeriod, reportType: reportType === 'bundle' ? '' : reportType, fileName: file.name, fileType: file.type, contentBase64, notes: page.querySelector('#upload-notes')?.value || '' }) });
        ...
        while (!result && retries < 3) {
          try { result = await submit(); }
            ...
            if (error.code === 'COMPANY_MISMATCH' && error.detectedCompanyKey) {
              const switchCompany = window.confirm(`检测到“${file.name}”的地区为 ${error.detectedCompanyName || companyNameByKey(error.detectedCompanyKey)}，与当前选择 ${error.selectedCompanyName || companyNameByKey(entryCompanyKey)} 不一致。\n\n确定：切换到检测地区并继续上传\n取消：保留当前地区并停止上传`);
              if (!switchCompany) throw new Error('已停止上传，请核对并重新选择地区');
              entryCompanyKey = error.detectedCompanyKey; companyKey = entryCompanyKey; state.uploadCompany = companyKey; setUploadCompany(companyKey, { confirmed: true }); retries += 1; continue;
            }
            if (error.code === 'GROUP_COMPANY_REQUIRED' && error.requiredCompanyKey === 'group') { entryCompanyKey = 'group'; retries += 1; continue; }
            if (error.code === 'PERIOD_MISMATCH' && error.detectedPeriod) {
              ...
              period = error.detectedPeriod; entryPeriod = period; state.uploadPeriod = period; setUploadPeriod(period, { confirmed: true }); retries += 1; continue;
            }
            throw error;
          }
        }
  ...
  page.querySelectorAll('[data-publish]').forEach(button => button.onclick = async () => {
    ...
    try { const result = await api(`/api/uploads/${button.dataset.publish}/publish`, { method: 'POST' }); const refreshText = result.consultantDirectoryRefresh?.requested ? '；已自动刷新顾问人事匹配' : result.consultantDirectoryRefresh?.error ? `；${result.consultantDirectoryRefresh.error}` : ''; showNotice(`已发布为当前版本${refreshText}`); await renderUploads(); }
