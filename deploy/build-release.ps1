...
try {
  foreach ($name in @(".dockerignore", "Dockerfile", "app.mjs", "app.test.mjs", "permission-center.test.mjs", "platform-auth.mjs", "platform-auth.test.mjs", "package.json", "package-lock.json", "README.md")) { Copy-Item -LiteralPath (Join-Path $projectDirectory $name) -Destination $staging }
