...
async function publishIntercompanyFixture(period = '2027-04') {
  ...
  for (const [region, company] of companies) {
    ...
    const reports = { trial_balance: { sourceSheet: '4月科目余额表', rows: trialRows, maxRow: trialRows.length, maxCol: 10 }, journal: { sourceSheet: '4月序时账', rows: journalRows, maxRow: journalRows.length, maxCol: 7 } };
...
before(async () => {
  child = spawn(process.execPath, ['app.mjs'], { cwd: projectDir, env: { ...process.env, NODE_ENV: 'test', PORT: String(testPort), DB_FILE: testDbPath, UPLOADS_DIR: testUploadsDir }, stdio: 'ignore' });
  let started = false;
  for (let i = 0; i < 40; i++) {
...
test('工资表固定为内部数据源且任何角色都不能整表浏览或导出', async () => {
  ...
  const backend = fs.readFileSync(path.join(projectDir, 'app.mjs'), 'utf8');
  assert.match(backend, /sourceOnlyReportTypes = new Set\(\[payrollStatementReportType\]\)/);
...
test('管理员可保存全局看板模块顺序，普通员工不可修改', async () => {
...
test('现金流量表金额下钻读取隐藏底稿并按现金流项目筛选', async () => {
  ...
  [header[0], header[1], header[2], header[4], header[5], header[6], header[17], header[18], header[19]] = ['日期', '凭证号', '摘要', '科目名称', '借方金额', '贷方金额', '备注', '现金流量表项目', '备注'];
...
test('集团顾问投入产出比联合工资表、营收明细和各公司序时账且保留来源', async () => {
  ...
  assert.notEqual(refreshed.payload.sourceRevision, result.payload.sourceRevision);
...
test('财务数据简报按集团与公司范围联合已发布报表自动取数', async () => {
  ...
  const forbiddenItem = await post('/api/analysis/financial-brief/secondary-items', { companyKey: 'gz', period, metricKey: 'expectedRevenue', name: '无权项目', amount: 100 }, 'viewer');
  assert.equal(forbiddenItem.response.status, 403);
  const rejectedResultItem = await post('/api/analysis/financial-brief/secondary-items', { companyKey: 'gz', period, metricKey: 'comprehensiveRevenueProfit', name: '不应新增', amount: 100 }, 'manager');
  assert.equal(rejectedResultItem.response.status, 400); assert.match(rejectedResultItem.payload.error, /营收综合利润不支持二级项目/);
  const rejectedAmount = await post('/api/analysis/financial-brief/secondary-items', { companyKey: 'gz', period, metricKey: 'expectedRevenue', name: '金额缺失' }, 'manager');
  assert.equal(rejectedAmount.response.status, 400); assert.match(rejectedAmount.payload.error, /金额格式无效/);
  const createdItem = await post('/api/analysis/financial-brief/secondary-items', { companyKey: 'gz', period, metricKey: 'expectedRevenue', name: '  重点签约项目  ', amount: '1,234.567' }, 'manager');
  assert.equal(createdItem.response.status, 201); assert.equal(createdItem.payload.item.name, '重点签约项目'); assert.equal(createdItem.payload.item.amount, 1234.57);
  assert.equal((await put('/api/analysis/financial-brief/secondary-items', { itemKey: createdItem.payload.item.itemKey, name: '越权修改', amount: 9 }, 'viewer')).response.status, 403);
  ...
  const updatedItem = await put('/api/analysis/financial-brief/secondary-items', { itemKey: createdItem.payload.item.itemKey, name: '本月重点签约项目', amount: -2560.4 });
  assert.equal(updatedItem.response.status, 200); assert.equal(updatedItem.payload.item.name, '本月重点签约项目'); assert.equal(updatedItem.payload.item.amount, -2560.4);
  const withItem = await request(`/api/analysis/financial-brief?company=gz&period=${period}`, 'viewer');
  assert.equal(withItem.payload.secondaryItems.length, 1); assert.equal(withItem.payload.secondaryItems[0].metricKey, 'expectedRevenue'); assert.equal(withItem.payload.secondaryItems[0].name, '本月重点签约项目');
  ...
  assert.equal(briefPermissions.children.find(item => item.key === 'module.financial_brief.notes.manage')?.name, '编辑二级项目明细');
...
test('各公司往来校验形成七家公司二十一组双边结果并严格隔离明细权限', async () => {
  ...
  assert.equal(summary.response.status, 200); assert.equal(summary.payload.companyCount, 7); assert.equal(summary.payload.combinationCount, 21); assert.equal(summary.payload.metrics.coveredCompanies, 7); assert.equal(summary.payload.scopeComplete, true); assert.equal(summary.payload.dataComplete, true);
  ...
  assert.match(stylesheet, /\.intercompany-matrix\{[^}]*min-width:790px/); assert.match(html, /id="intercompany-reconciliation-page"/);
});
