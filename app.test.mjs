...
test('集团顾问投入产出比联合工资表、营收明细和各公司序时账且保留来源', async () => {
  ...
  assert.equal(byName['詹志坚'].expenseDetails[0].secondaryAccount, '差旅费');
  assert.equal(byName['张莎莎'].expenseDetails.length, 1);
  assert.equal(byName['张莎莎'].expenseDetails[0].secondaryAccount, '业务招待费');
  assert.deepEqual(Object.fromEntries(result.payload.reimbursementAccounts.map(item => [item.name, { amount: item.amount, detailCount: item.detailCount }])), { '差旅费': { amount: 3000, detailCount: 1 }, '业务招待费': { amount: 1200, detailCount: 1 } });
