...
- 生产复用现有 AQLLM 智谱兼容地址与 `glm-5-turbo`，但不得直接把 `/data/config/aqllm/LLM.env` 整体注入财务容器。上线时由管理员在服务器本地、无标准输出的流程中，把所需地址、模型和 Key 写入财务项目专用 `/data/secrets/wecom-finance-report-board/report-board.env` 的 `UPLOAD_MAPPING_LLM_*` 变量，并保持既有 Secret 权限；就绪检查在启用状态下强制 HTTPS、非占位 Key 和 1–15 秒超时。
