...
## 37. 分析模块拖动与落位锁定候选（1.1.48，待部署）

- 分析模块六点拖动手柄提高常态对比度，并提升父卡片和覆盖层的命中层级；鼠标进入手柄、卡片边缘或相邻网格时保持显示，触屏端同样保留清晰状态。
- 指针移动达到 `5px` 后才开始重排，拖动过程中使用独立浮动标题跟随指针，并在目标卡片上显示前置/后置落位线；被移动卡片不再遮挡 `elementFromPoint` 的目标计算。
- 松手和取消事件同时由手柄与窗口捕获层兜底，避免卡片换位后原手柄收不到 `pointerup` 而残留拖动态。取消操作恢复原顺序；保存失败在当前页面动画回滚，不再通过整页刷新掩盖错误。
- 顺序提交期间全部拖动手柄禁用并标记页面忙碌，防止并发覆盖。成功提示明确“已应用于所有公司和员工”；接口仍校验完整板块集合并将顺序写入全局 `analysis_block_order`。
- 本版本没有数据库结构迁移。生产当前仍为 `1.1.47`，候选上线前须执行完整 Node 测试、前端语法检查、部署检查和桌面/移动端拖放验收；回滚仅需恢复上一镜像与 Compose，数据库完整性正常时不得覆盖业务数据。

## 38. 现金流量表现金收支分析生产发布（1.1.49）

- 现金流量表标题区新增“现金收支分析”入口，弹窗与资产负债分析保持同一结构：左侧页签滑动切换钱的来源/钱的去向项目金额表，右侧同步切换大幅环形图。
- 数据只读取当前公司、期间和当前已发布现金流量表；通过“累计现金收支明细”标题及“增减项、项目、金额”三层语义锚点定位，不使用固定单元格。不同月份新增、减少或改名的项目按源表原文和顺序展示，未知项目不丢弃。
- “期初、加、增加、流入”归入来源，“减、减少、流出”归入去向；金额兼容数值、千分位、元、括号负数和短横线，图表按资金规模绝对值计算占比。缺少分析区或合计行时明确提示，不复用其他月份数据。
- 表格行、图表色块和直连标签支持鼠标与键盘三方联动，选中项动画放大并在圆心显示占比和金额；继续复用去除 Chromium 黑色 SVG 焦点框的清晰焦点样式。弹窗打开、手动刷新、每 60 秒及页面恢复可见时均按当前发布批次重算。
- `2026-09-04 10:52 CST` 使用提交 `1b13902` 的固定发布包上线；包 SHA256 为 `A08CACB9EDC9AFF38212B366A8C3642977BBBD8FE1A884800134AF321F1691F8`。本地及服务器 Docker tests 均为 `103/103`，正式镜像为 `aqllm/finance-report-board:1.1.49`，镜像 ID 为 `sha256:fe78daf500c692b76da88ff09c680ec95d8df70c5589e80127544fef02b45fbc`。
- 容器为 `healthy`，回环和公网健康接口均返回 `1.1.49/platform`；Nginx、运行时隔离、生产平台登录及广州 2026-07 真实现金收支分析弹窗验收通过，生产静态 `app.js`、现金流模块和共享分析样式哈希与候选包完全一致。
- SQLite 完整性发布前后均为 `ok`，核心业务事实保持 8 家公司、235 个上传批次、235 份报表快照和 13,061 条报表行。本版本不修改数据库表结构；浏览验收只新增审计日志。
...
## 40. 企业微信精确范围瞬态重试候选（1.1.51，已安全回滚）
...
- `2026-09-04 14:02 CST` 使用提交 `f0dfef99a34d6d904912d020734cae1122db9349` 的固定发布包完成生产发布；包 SHA256 为 `CCF4BAB81FF9A528E2CC2D3FC498C33183F1F7AD3D0C8BBC3B1DFA9C81DDC4EC`。服务器 Docker tests 为 `104/104`，正式镜像为 `aqllm/finance-report-board:1.1.53`，镜像 ID 为 `sha256:7d8f4bc03454ee287846c5ff505f71bd1de8e7e26f185cb795b9fa8cbda9ccb9`。
- 容器为 `healthy`，回环与公网健康接口均返回 `1.1.53/platform`；Nginx、运行时隔离及真实平台登录通过。生产管理员顾问页面显示“名单显示设置”、不再显示导出按钮，弹窗列出 31 项且全部保持显示；验收只打开后取消，数据库没有写入隐藏名单。浏览器无警告或错误；普通用户不返回设置数据、无入口且保存接口返回 `403` 的隔离由同一服务器测试镜像覆盖。
...
## 44. 顾问公司口径、人事日期与统一投入配置候选（1.1.59，待部署）

- 花名册读取改为两组相互独立的精确范围：“在职”和“离职”工作表仅读取 `D:F` 的所属公司、姓名、英文名；“离职”工作表额外只读 `B:B` 的离职日期，并按同一行号合并。任何起始列、宽度或表头不符合约定时安全停止，不读取 `C` 列离职原因，也不回退到整表或宽范围。
- 脱敏目录升级为 schema 2，人员字段白名单仅为 `name`、`englishName`、`companyName`、`employmentStatus`、`departureDate`。入职日期继续仅来自工资表；姓名旁“新”“离职”使用同尺寸圆形标签，鼠标悬停、键盘聚焦和触屏点击均可查看相应日期，提示按视口自动上下翻转并在滚动时关闭。
- 明细列“业绩归属”统一改为“所属公司”，只采用企微花名册数据，缺失时显示“待补充”。营收表未上传或不可解析显示“缺营收”；报表已发布且该顾问当月营收为空或为零显示“无营收”；有效营收显示“已匹配”。“无营收”不计入来源缺失提示。
- 匹配状态列及诊断字段只向权限管理员返回和渲染。普通员工接口不返回 `matchStatus`、规范姓名、候选报销科目及明细数组，页面也没有匹配状态表头、筛选入口或报销科目配置控件。
- 基本工资、提成、报销费用、投流消耗四项投入开关与报销二级科目改为按期间保存的管理员统一设置。新期间默认基本工资、报销费用、投流消耗开启，提成关闭，并默认选择该期间全部可用报销科目；管理员修改草稿后必须显式“应用于全员”。已保存但重传后消失的科目被忽略并提示，不自动纳入后来新增科目。设置写入既有 `app_settings`，并进入 `sourceRevision` 以触发在线刷新；审计只记录管理员、期间、科目名称和启用项目，不记录序时账正文。
- 报销科目浮层采用紧凑筛选按钮与 WPS 风格分区，包含搜索、全选、清空、金额/笔数、取消和保存；只有选择变化时可保存。顾问表头、筛选按钮、列宽、字号和对齐已统一，桌面优先在内容区完整展示，窄屏只允许表格容器自身横向滚动，不挤压左侧导航。
- 本地完整回归为 `108/108`，服务端、前端和同步脚本语法检查、部署模板检查及差异检查均通过。真实浏览器样例中，`1280` 与 `1440` 宽屏的页面宽度等于视口且表格无横向滚动；`820` 与 `390×844` 时整页仍无横向溢出，仅顾问表格容器独立滚动。管理员标签日期、设置草稿与浮层碰撞检查通过，普通员工页面确认没有匹配状态列、候选科目或管理员设置入口，两个浏览器控制台均无警告或错误。
- 本版本没有数据库结构迁移。生产当前仍为 `1.1.54`；上线前须固定候选包、在服务器 Docker 内完成全量测试，创建 SQLite 一致性备份，并保留现有财务专用 CLI、独立 HOME/XDG_CONFIG_HOME、五个 systemd unit 及三个已启用监听。先用候选同步脚本完成纯 import 和一次手工同步，确认 schema 2 字段白名单、文件 `0600`、所有者 `20117:20117` 后才可切换正式容器。
- 任一同步、测试、权限、Nginx、运行时隔离或页面验收异常都应停止并恢复 `1.1.54` 源码、Compose 与镜像；同时恢复发布前保存的同步脚本和 unit 文件。数据库完整性正常时不得以旧备份覆盖业务数据；新增的按期间设置位于既有设置表，旧版会忽略，不需要回滚数据库。

## 45. 营收累计子表字段边界修复候选（1.1.60，待部署）

- 营收累计解析不再把最后一个已识别子表延伸至工作表最右列。每张累计表必须先按累计区标题和子表标题定位，再从子表起始列按既定顺序匹配完整字段表头；实际列宽截止到最后一个必需字段。
- 历史月份可以只包含部分累计子表。例如生产 2026 年 6 月文件只存在 `L1`、`L2`、`L4`，系统保留缺表诊断，但不会把右侧非累计项目明细映射进 `L4`。
- 累计解析结果新增内部解析器版本。已上传、已发布且仍保留原始 Excel 的旧快照会在读取时自动按新规则重解析，不要求业务人员重新上传；原始文件不存在或解析失败时仍保留旧结果，不静默伪造数据。
- 本地完整回归为 `108/108`。另以生产 2026 年 6 月原始营收 Excel 在隔离数据目录实测：只识别 `L1`、`L2`、`L4`，列宽依次为 4、5、5，`L4` 表头为“来源（一级）、月份、预计营收、营收占比、项目数量”，右侧非累计明细未进入结果。
- 生产当前仍为 `1.1.54`，本节只是候选说明，未授权部署。上线应与 1.1.59 以来的未部署候选统一冻结、备份和验收；异常时按最近已确认生产版本恢复源码、Compose 与镜像，数据库完整性正常时不得以旧备份覆盖业务数据。

## 46. 上传时模型辅助结构映射候选（1.1.61，待部署）

- 采用“规则主判、模型辅助、程序复核”三段式流程。现阶段只在营收统计年度累计结构出现缺表或歧义时触发，规则已经完整识别时不调用模型；后续其他报表必须先定义可验证字段契约，再逐项接入，禁止用模型输出直接替代财务取数规则。
- 发给模型的内容仅包含目标工作表的通用标识、期间、累计候选标题、字段名及行列坐标。数值单元格、客户、项目内容、员工/顾问内容、完整工作表和原始文件均不进入请求；请求和响应正文不得写入日志。
- 模型只建议年度、L1/L2/L2-1/L3/L4/L5/L6 表型、标题行、表头行、起始列和置信度。后端要求置信度至少 0.9，并重新验证年度分区、允许表型、标题坐标是否来自已发送的结构白名单、表头字段顺序、工作表边界和非空数据区；任何一项失败即忽略建议。
- 超时、非 2xx、非 JSON、越界或低置信度均不阻断上传，也不生成伪造表。上传响应与原始快照只记录 `applied/confirmed_no_change/failed/timeout/unavailable` 状态、模型名及成功采用的表型，不保存模型原始回答。
- 生产复用现有 AQLLM 智谱兼容地址和项目专用 Key，但不得直接把 `/data/config/aqllm/LLM.env` 整体注入财务容器。最初候选采用 `glm-5-turbo`，最终上线模型以第 48 节的脱敏探针结果为准。上线时由管理员在服务器本地、无标准输出的流程中，把所需地址、模型和 Key 写入财务项目专用 `/data/secrets/wecom-finance-report-board/report-board.env` 的 `UPLOAD_MAPPING_LLM_*` 变量，并保持既有 Secret 权限；就绪检查在启用状态下强制 HTTPS、非占位 Key 和 1–15 秒超时。
- 自动化覆盖规则完整时零模型调用、脱敏结构请求、有效建议采纳、越界建议拒绝和模型 503 降级。生产当前仍为 `1.1.54`，本候选没有修改生产配置或调用生产模型；正式启用前还需在候选容器内用脱敏样例做一次请求验证，并检查容器日志不含请求正文或 API Key。

## 47. 顾问英文名与入职日期列顺序优化候选（1.1.62，待部署）

- 顾问投入产出明细前四列固定为“顾问、英文名、入职日期、所属公司”；表头、筛选排序配置、`colgroup` 和正文单元格使用同一顺序，管理员与普通员工均保持一致，普通员工继续不返回或渲染末列匹配状态。
- 姓名、英文名、所属公司左对齐，入职日期居中；列宽仍由语义 class 控制，不依赖英文名与日期的固定序号。桌面内容区应完整展示，窄屏仅允许表格容器自身横向滚动；人员状态日期提示继续锚定姓名旁标签并限制在视口内。
- 本版本没有数据库、接口或企微同步范围变更，保留 `1.1.59` 顾问口径与 `1.1.60`、`1.1.61` 的累计解析和上传映射候选。生产当前仍为 `1.1.54`，应随未部署候选统一备份、服务器全测和浏览器验收；异常时恢复最近确认生产版本，数据库完整性正常时不得覆盖业务数据。
- 本地完整回归 `109/109`，服务端/前端语法、部署配置和差异检查均通过。真实浏览器中管理员与普通员工的前四列表头及正文均为上述顺序；普通员工仍无“匹配状态”列和报销配置入口。`1280×720` 下页面与表格均无横向溢出、表头无重叠；`390×844` 下整页无横向溢出，仅表格容器以 `overflow-x:auto` 独立滚动。入职日期提示完整位于视口内且不遮挡日期列，浏览器控制台无警告或错误。

## 48. 营收累计、模型辅助与顾问口径统一生产发布（1.1.62）

- `2026-09-04 17:09 CST` 使用提交 `3674342` 的固定发布包完成生产发布；发布包 SHA256 为 `4AF222C8D85896D14435DEFE0FBDE4731318EB3EACF8B1F81097A0F416CAC13A`。服务器 Docker tests 为 `109/109`，正式镜像为 `aqllm/finance-report-board:1.1.62`，镜像 ID 为 `sha256:855444b5e9a63fd6712af7e0272326ef0debd983d4e1ea3731294781d39cf73c`。
- 本次统一上线 `1.1.59` 至 `1.1.62`：顾问公司、人事日期和全员投入口径；营收累计子表字段边界与旧快照动态重解析；上传时“规则主判、模型辅助、程序复核”；顾问、英文名、入职日期、所属公司的固定列顺序。生产 2026 年 6 月累计数据会按字段标题收口，不再把右侧非累计区域并入 L4。
- 模型配置没有把共享 `/data/config/aqllm/LLM.env` 整体挂载给财务容器，只将接口、模型和 Key 写入财务项目专用受限环境文件。`glm-5-turbo` 的脱敏合成探针耗时约 26 秒，超过应用允许的 15 秒上限，因此生产改用同一智谱接口下实测可在上限内完成的 `glm-4-flash`；最终脱敏探针返回 `ready/completed`，请求不含业务数值、客户、项目或人员内容，日志未记录请求正文或密钥。
- 候选镜像先以只读数据库生成 42 项仅含 `name` 的最小顾问清单，再完成同步脚本纯 import 和一次手工同步。脱敏目录为 schema 2、42 人，字段严格为 `name`、`englishName`、`companyName`、`employmentStatus`、`departureDate`；清单、目录和状态文件均为 `0600`、`20117:20117`。
- 容器为 `healthy`，回环和公网健康接口均返回 `1.1.62/platform`；Nginx、运行时隔离和生产登录引导页通过。真实浏览器可见版本 `v1.1.62`，控制台无 warning/error。三个企微监听继续为 `enabled/active`，五个 systemd unit 与发布前逐文件一致。
...
- 页面顶部栏固定于最上层，运行时使用 `ResizeObserver` 将其真实高度写入统一 `--app-header-height`；侧栏、页面留白和局部管理面板均以该值计算位置。全站表头随表格自然滚动，避免进入顶部栏；宽表只在自身容器横向滚动，主内容和侧栏不再侵入品牌区或撑宽整页。
...
## 51. 动态营收解析与分公司营收趋势生产发布（1.1.67）

- 营收统计表改为按区域标题、字段表头和实际数据边界动态识别月度及累计子表；子表数量、名称、位置和列宽不再写死。上传期间以用户选定期间为主，旧版、改名或新增子表可以按当月实际结构更新；模型只在规则低置信度时接收脱敏结构标签并接受程序复核。
- 分公司新增“营收趋势分析”，从已发布集团营收统计原文件的总营收明细表动态取数，按期间筛选并严格限定“业绩归属”为当前地区。管理员可以用已识别表头配置最多 8 组全局统计组合，全员统一展示；历史趋势按各月“预计营收”生成并支持滑动切换，缺失月份不补零。
- `2026-09-04 21:05 CST` 使用提交 `4c636f5` 的固定发布包完成生产发布；发布包 SHA256 为 `1DCE1C6DB240F41330E16752655EBE24AC62B98AA8A691787E4B30FCCF3016DF`。服务器 Docker tests 为 `109/109`，正式镜像为 `aqllm/finance-report-board:1.1.67`，镜像 ID 为 `sha256:59f652123ea742d4139f775cb3ec2fe0763909880f9e0c17f0a3e326fbf5f196`。
