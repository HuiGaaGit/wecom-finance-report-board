...
- 最终发布包：`artifacts/final-1.1.27/wecom-finance-report-board-1.1.27.zip`；SHA256：`C16AE1D41D89D6E68A7F08740B96D32F869F23CA6C82958558AF501874ACF978`。本地完整回归 `89/89`，真实浏览器桌面与手机端验收通过。Docker 依赖阶段提供仅构建时使用的 Python、make 和 g++，确保 `better-sqlite3` 在预编译包不可用时仍可可靠构建，最终运行镜像不包含这些工具。
